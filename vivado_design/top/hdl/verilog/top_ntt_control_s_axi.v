...
#(parameter
    ...
    output wire [63:0]                   zetas_forward,
...
localparam
    ...
    ADDR_ZETAS_FORWARD_DATA_0 = 6'h28,
    ADDR_ZETAS_FORWARD_DATA_1 = 6'h2c,
    ADDR_ZETAS_FORWARD_CTRL   = 6'h30,
    WRIDLE                    = 2'd0,
    WRDATA                    = 2'd1,
    WRRESP                    = 2'd2,
    WRRESET                   = 2'd3,
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_ZETAS_FORWARD_DATA_0: begin
                    rdata <= int_zetas_forward[31:0];
                end
                ADDR_ZETAS_FORWARD_DATA_1: begin
                    rdata <= int_zetas_forward[63:32];
                end
            endcase
        end
    end
...
assign zetas_forward     = int_zetas_forward;
// int_interrupt
always @(posedge ACLK) begin
    if (ARESET)
        int_interrupt <= 1'b0;
...
// int_zetas_forward[31:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_zetas_forward[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_ZETAS_FORWARD_DATA_0)
            int_zetas_forward[31:0] <= (WDATA[31:0] & wmask) | (int_zetas_forward[31:0] & ~wmask);
    end
end

// int_zetas_forward[63:32]
always @(posedge ACLK) begin
    if (ARESET)
        int_zetas_forward[63:32] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_ZETAS_FORWARD_DATA_1)
            int_zetas_forward[63:32] <= (WDATA[31:0] & wmask) | (int_zetas_forward[63:32] & ~wmask);
    end
end
