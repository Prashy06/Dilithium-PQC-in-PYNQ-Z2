...
entity top_ntt_control_s_axi is
generic (
...
end entity top_ntt_control_s_axi;
...
-- 0x24 : reserved
-- 0x28 : Data signal of zetas_forward
--        bit 31~0 - zetas_forward[31:0] (Read/Write)
-- 0x2c : Data signal of zetas_forward
--        bit 31~0 - zetas_forward[63:32] (Read/Write)
-- 0x30 : reserved
-- (SC = Self Clear, COR = Clear on Read, TOW = Toggle on Write, COH = Clear on Handshake)

architecture behave of top_ntt_control_s_axi is
    ...
    constant ADDR_ZETAS_FORWARD_DATA_0 : INTEGER := 16#28#;
    constant ADDR_ZETAS_FORWARD_DATA_1 : INTEGER := 16#2c#;
    constant ADDR_ZETAS_FORWARD_CTRL   : INTEGER := 16#30#;
    ...
    signal int_zetas_forward   : UNSIGNED(63 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_ZETAS_FORWARD_DATA_0 =>
                        rdata_data <= RESIZE(int_zetas_forward(31 downto 0), 32);
                    when ADDR_ZETAS_FORWARD_DATA_1 =>
                        rdata_data <= RESIZE(int_zetas_forward(63 downto 32), 32);
                    when others =>
                        NULL;
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    zetas_forward        <= STD_LOGIC_VECTOR(int_zetas_forward);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_zetas_forward(31 downto 0) <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_ZETAS_FORWARD_DATA_0) then
                    int_zetas_forward(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_zetas_forward(31 downto 0));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_zetas_forward(63 downto 32) <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_ZETAS_FORWARD_DATA_1) then
                    int_zetas_forward(63 downto 32) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_zetas_forward(63 downto 32));
                end if;
            end if;
        end if;
    end process;
